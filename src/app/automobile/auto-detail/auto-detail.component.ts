...
export class AutoDetailComponent {
  ...
  alreadyMadeOffer: boolean = false;
  ...
  buyAutomobile() {
    const comment = prompt(
      'Write down your offer comment',
      'I am interested to buy this car'
    );
    if (comment) {
      this.autoCrudService
        .makeOffer(this.autoId, { comment })
        .subscribe((res) => {
          alert('Offer sent!');
          this.alreadyMadeOffer = true;
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params: ParamMap) => {
      ...
      this.autoCrudService.getAutoById(id).subscribe((response) => {
        ...
        this.alreadyMadeOffer =
          this.autoDetails.offers?.findIndex(
            (offer: any) => offer.userId === currentUserId
          ) > -1;

        if (currentUserId === ownerId || this.alreadyMadeOffer) {
          this.shouldShowOfferBtn = false;
        }
      });
    });
